    ...
    def _tick(self) -> None:
        ...
        if not connected or self._status is None:
            ...
            status.values = [
                ...
                KeyValue(
                    key="battery_voltage", value=f"{self._status.battery_voltage:.2f}"
                ),
                KeyValue(
                    key="linear_velocity", value=f"{self._status.linear_velocity:.3f}"
                ),
                KeyValue(
                    key="angular_velocity", value=f"{self._status.angular_velocity:.3f}"
                ),
            ]
