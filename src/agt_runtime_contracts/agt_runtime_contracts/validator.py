...
def resolve_relative_path(site_root: Path, raw: str) -> Path:
    resolved, issue = _resolve_relative_path(Path(site_root), raw)
    if issue is not None or resolved is None:
        problem = issue or ValidationIssue("PATH", f"invalid Site Package path: {raw}")
        raise ValueError(f"{problem.code}: {problem.message}")
    return resolved
...
def navigation_image_asset(
    site_root: Path,
    manifest: Mapping[str, Any],
) -> tuple[str, Path]:
    raw, path, issue = _navigation_image_asset(Path(site_root), manifest)
    if issue is not None or raw is None or path is None:
        problem = issue or ValidationIssue("NAVIGATION_MAP", "invalid navigation map image")
        raise ValueError(f"{problem.code}: {problem.message}")
    return raw, path
...
def load_hashes(site_root: Path, manifest: Mapping[str, Any]) -> dict[str, str]:
    ...
    hashes_path = resolve_relative_path(Path(site_root), str(hashes_raw))
    hashes_doc = load_yaml(hashes_path)
    hashes = hashes_doc.get("hashes")
    if not isinstance(hashes, Mapping):
        raise ValueError("HASHES_FILE: hashes.yaml must contain a 'hashes' mapping")
