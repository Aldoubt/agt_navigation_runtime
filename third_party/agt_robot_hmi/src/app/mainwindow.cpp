...
void MainWindow::setupUi() {
  ...
  connect(nav_goal_table_view_, &NavGoalTableView::signalSendTaskChain,
          [this](const std::string &json) { PUBLISH(MSG_ID_TASK_CHAIN, json); });
  connect(nav_goal_table_view_, &NavGoalTableView::signalCancelTaskChain,
          [this]() { PUBLISH(MSG_ID_TASK_CANCEL, std::string("cancel")); });
  SUBSCRIBE(MSG_ID_TASK_STATUS, [this](const std::string &status) {
    nav_goal_table_view_->UpdateTaskStatus(status);
  });
