...
void NavGoalTableView::StartTaskChain(bool is_loop) {
  ...
  emit signalSendTaskChain(document.dump());
}
...
void NavGoalTableView::UpdateTaskStatus(const std::string &status_json) {
  try {
    const auto status = nlohmann::json::parse(status_json);
    const std::string state = status.value("state", "UNKNOWN");
    for (int row = 0; row < table_model_->rowCount(); ++row) {
      if (auto *label = static_cast<QLabel *>(indexWidget(model()->index(row, 1)))) {
    ...
    LOG_ERROR("invalid HMI task status: " << error.what());
  }
}
