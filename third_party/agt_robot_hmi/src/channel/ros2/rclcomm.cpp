...
bool rclcomm::Start() {
  ...
  topology_map_update_publisher_ = node->create_publisher<topology_msgs::msg::TopologyMap>(
      ...
      rclcpp::QoS(rclcpp::KeepLast(1)).reliable().transient_local());
  hmi_task_chain_publisher_ = node->create_publisher<std_msgs::msg::String>(
      "/agt/hmi/task_chain", 10);
  hmi_task_cancel_publisher_ = node->create_publisher<std_msgs::msg::String>(
      "/agt/hmi/task_cancel", 10);
  hmi_task_status_subscriber_ = node->create_subscription<std_msgs::msg::String>(
      "/agt/hmi/task_status", 20,
      std::bind(&rclcomm::hmiTaskStatusCallback, this, std::placeholders::_1));
  ...
  SUBSCRIBE(MSG_ID_TASK_CHAIN, [this](const std::string &json) {
    PubHmiTaskChain(json);
  });
  SUBSCRIBE(MSG_ID_TASK_CANCEL, [this](const std::string &) {
    CancelHmiTaskChain();
  });
  SUBSCRIBE(MSG_ID_SET_NAV_GOAL_POSE, [this](const basic::RobotPose& pose) {
    std::cout << "legacy single goal requested: " << pose << std::endl;
  });
...
void rclcomm::PubHmiTaskChain(const std::string &json) {
  std_msgs::msg::String message;
  message.data = json;
  hmi_task_chain_publisher_->publish(message);
}

void rclcomm::CancelHmiTaskChain() {
  std_msgs::msg::String message;
  message.data = "cancel";
  hmi_task_cancel_publisher_->publish(message);
}

void rclcomm::hmiTaskStatusCallback(const std_msgs::msg::String::SharedPtr msg) {
  // Keep the vendor UI message bus boundary intact; the task adapter owns the
  // authoritative state and the UI can consume this status in a later model.
  std::cout << "HMI task status: " << msg->data << std::endl;
  PUBLISH(MSG_ID_TASK_STATUS, msg->data);
}
